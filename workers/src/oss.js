...
export async function signUpload(env, contentType, sizeLimit = 100 * 1024 * 1024) {
  const bucket = env.OSS_BUCKET;                       // kcyd-kb/ 前缀
  const region = env.OSS_REGION;                       // e.g. oss-cn-hangzhou
  const dir = 'kb/';
  ...
  const sig = b64bytes(await hmacSha1(
    new TextEncoder().encode(env.OSS_ACCESS_KEY_SECRET), policyB64
  ));
